...
export function isSupportedBlockType(type: string) {
  return supportedBlockTypes.has(type);
}

export function getClassAttributes(blockObj: any) {
  let classes = [];
...
export function getStyles(blockObj: any) {
  const color = getColor(blockObj.annotations?.color);
  return getStyleString(color);
}

export function getStyleString(styleObj: any) {
  if (!styleObj) {
    return "";
  }
...
// export function getEmoji(block: any) {}

export function getPlainText(textObj: any) {
  if (typeof textObj?.type !== "string") {
    return;
  }

  const type = textObj.type;
  const plainText = textObj[type].map((text: any) => text.plain_text).join("");

  return plainText;
}

export function getPostDate(created: string, edited: string, userDateObj: any) {
  const isValidUserDate = (userDateObj: any) => {
    if (
      userDateObj === null ||
  ...
  const getUserDate = (userDateObj: any) => {
    if (
      userDateObj === null ||
      userDateObj?.type !== "date" ||
      userDateObj[userDateObj?.type] === null
